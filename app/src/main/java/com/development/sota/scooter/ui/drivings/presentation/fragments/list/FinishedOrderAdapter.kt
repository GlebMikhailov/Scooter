...
class FinishedOrderAdapter(val data: List<OrderWithStatus>, val context: Context): RecyclerView.Adapter<FinishedOrderAdapter.FinishedOrderViewHolder>() {
    private val dateFormatter = SimpleDateFormat("dd.MM.yyyy", Locale.getDefault())

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FinishedOrderAdapter.FinishedOrderViewHolder
            = FinishedOrderViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.item_finished_order, parent, false)
    )

    override fun onBindViewHolder(holder: FinishedOrderAdapter.FinishedOrderViewHolder, position: Int) =
        holder.itemView.run {
            textViewItemFinishedOrderDate.text = dateFormatter.format(data[position].order.parseStartTime())
            textViewItemFinishedOrderAmount.text = String.format("%.2f", data[position].order.cost).plus(" ₽")
